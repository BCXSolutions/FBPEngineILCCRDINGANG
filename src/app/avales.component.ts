 ...
 * Form: Buscar
 */
export class AvalesComponent implements OnInit, AfterViewChecked
{
	// Nombre de la pagina.
	...
	constructor (private hostService: CmWsHostService
		...
		, private crdRs200131Gar: CRD_RS_200_131_GAR
		, private crdRs200151Gar: CRD_RS_200_151_GAR
		){}
	/**
	 * Inicializamos todo.
	 */
	ngOnInit()
	{
		...
		this.tableCols = [
			{ prop:'wss_gar_atr', name:'Rut', width:'100', headerClass:'gridHeader'},
			{ prop:'wss_nom_cli', name:'Nombre', width:'200', headerClass:'gridHeader'},
			{ prop:'wss_gar_tip', name:'Tipo', width:'80', headerClass:'gridHeader'},
			{ prop:'wss_gar_pct_format', name:'%', width:'120', headerClass:'gridHeader', cellTemplate: this.rightTmpl, comparator: this.utilService.sortDecimal.bind(this)},
			{ prop:'wss_gar_tip_corr', name:'% Avalado', width:'100', headerClass:'gridHeader', cellTemplate: this.rightTmpl, visible:false},
		];
	 ...
	 */
	private crdRs200131GarCall(): void
	{
		...
		this.waitShow = true;
		// Invocamos el WS.
		this.crdRs200131Gar.call(
			  (value) => this.crdRs200131GarResult(value)
	...
	/**
	 * Callback invocado por this.crdRs200131Gar.call.
	 * @param wsResult Parametros de salida, mensaje de error.
	 */
	crdRs200131GarResult(wsResult :CmWsResult): void
	{
		...
		if(wsResult.getReturnValue()== 0){
			...
			this.crdRs200151GarCall();
			this.utilService.alert(this.dialog, "Aval eliminado con exito");
	 ...
	 * Llamamos al Web Service.
	 */
	private crdRs200151GarCall(): void
	{
		/* Mover los datos de la pantalla a los parametros del Web Service. */ 
		let wss_num_opr :string = this.txtNumeroOperacion.value; 
		let wss_usercode :string = this.user_logueado;
		...
		this.waitShow = true;
		// Invocamos el WS.
		this.crdRs200151Gar.call(
			  (value) => this.crdRs200151GarResult(value)
			, (value) => this.processFault(value)
			, wss_num_opr
			, wss_usercode
		);
	...
	/**
	 * Callback invocado por this.crdRs200151Gar.call.
	 * @param wsResult Filas de la tabla, parametros de salida, mensaje de error.
	 */
	crdRs200151GarResult(wsResult :CmWsResult): void
	{
		// Desactivamos el simbolo de progress.
		this.waitShow = false;
		let wss_result_msg:any =wsResult.getResultString('wss_result_msg') ;
		...
		// } else {
			...
			wsResult.formatDecAdd('wss_gar_pct', 5);
			// Obtenemos las filas.
	...
	cmdNuevo_click()
	{
		...
		//this.contextService.setUserData("bcxRut",this.utilService.toRut(this.bcxRut));
		this.contextService.setUserData("txtNumeroOperacion",this.txtNumeroOperacion.value);

		this.router.navigate(['/nuevoaval']);
	...
	cmdEliminar_click(): void
	{
		...
		if(this.tableSelected[0]=="" || this.tableSelected[0] == null) {
			this.utilService.alert(this.dialog, 'Debe seleccionar un Aval');
		} else {
			this.waitShow = false;
  ...
  private deleteResult(result: number) :void  {
	...
	private tableFilter(value :string): void
	{
		...
		const temp = this.tableRowsTemp.filter(d => {
			return !val
				|| this.utilService.hayVal(d.wss_gar_atr, val)
				|| this.utilService.hayVal(d.wss_nom_cli, val)
				|| this.utilService.hayVal(d.wss_gar_tip, val)
				|| this.utilService.hayVal(d.wss_gar_pct_format, val)
	...
	{
		this.txtNumeroOperacion.valueChanges.subscribe((value) => {
			this.utilService.bcxNumeroInit(this.txtNumeroOperacion);
		});
