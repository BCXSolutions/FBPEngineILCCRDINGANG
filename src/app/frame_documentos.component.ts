...
@Component({
	selector: 'my-form',
	templateUrl: 'frame_documentos.component.html'
})
/**
 * Form: Buscar
 */
export class frameDocumentosComponent implements OnInit
{
	// Nombre de la pagina.
	pageName: string = '';
	...
	ngOnInit()
	{
		...
		this.numeroOperacion = this.contextService.getUserData("numeroOperacion");
		...
		console.log('this.user_logueado: ', this.user_logueado);

		this.cargar_documentos_preingreso();
	...
	private crdRs200111OgarCall(): void
	{
		...
		let wss_num_opr :string = this.numeroOperacion;
	...
	cargar_documentos_preingreso() {

		this.url = this.hostService.getHost() + 'BCXDOCOPE/index.xhtml?setUsuario='+this.user_logueado+'&setPassword='+this.pass_logueado+'&setModo=CON&nOperacion='+this.numeroOperacion+"&nSolicitud="+this.numeroSolicitud+"&codProd=FNA";
